...
from cloud_panel.routes.agent_accounts import register_agent_account_routes
from cloud_panel.routes.agent_auth import register_agent_auth_routes
from cloud_panel.routes.agent_dashboard import register_agent_dashboard_routes
from cloud_panel.routes.agent_monitoring import register_agent_monitoring_routes
from cloud_panel.routes.agent_subagents import register_agent_subagent_routes
from cloud_panel.routes.agent_router_views import register_agent_router_view_routes
from cloud_panel.routes.dashboard import register_dashboard_routes
from cloud_panel.routes.router_groups import register_router_group_routes
from cloud_panel.routes.routers import register_router_routes
from cloud_panel.routes.system import register_system_routes
from cloud_panel.routes.updates import register_update_routes
from cloud_panel.routes.wireguard import register_wireguard_routes
...
from cloud_panel.services.dashboard import build_dashboard_data
from cloud_panel.services.wireguard import parse_ubuntu_wireguard_peers as _parse_ubuntu_wireguard_peers_service
from cloud_panel.services.wireguard import import_peers_from_ubuntu as _import_peers_from_ubuntu_service
from cloud_panel.services.wireguard import (
    clean_networks as _clean_networks_service,
    next_tunnel_ip as _next_tunnel_ip_service,
    peer_config as _peer_config_service,
    random_api_credentials as _random_api_credentials_service,
    wg_keypair as _wg_keypair_service,
)
from cloud_panel.services.router import provision_router_automatically as _provision_router_automatically_service
from cloud_panel.services.router import (
    mikrotik_api_bootstrap_script as _mikrotik_api_bootstrap_script_service,
    mikrotik_script as _mikrotik_script_service,
)
from cloud_panel.services.router import (
    router_overview as _router_overview_service,
)
from cloud_panel.services.router_helpers import (
    api_find_by_comment as _api_find_by_comment,
    api_find_by_name as _api_find_by_name,
    ros_quote as _ros_quote,
    routeros_bool as _routeros_bool,
)
...
def init_db():
    ...
    for column, definition in user_profile_migrations:
        if not has_column(con, "users", column):
            con.execute("ALTER TABLE users ADD COLUMN %s %s" % (column, definition))

    agent_profile_migrations = [
        ("phone", "TEXT NOT NULL DEFAULT ''"),
        ("email", "TEXT NOT NULL DEFAULT ''"),
    ]
    for column, definition in agent_profile_migrations:
        if not has_column(con, "agent_accounts", column):
            con.execute("ALTER TABLE agent_accounts ADD COLUMN %s %s" % (column, definition))
...
def _router_remove_cloud_pf_rules(api):
    # Rebuild only the NAT rules managed by the panel. RouterOS firewall
    # filter rules are never created, removed, or modified by this project.
    for row in api.talk("/ip/firewall/nat/print"):
        comment = str(row.get("comment", "") or "")
        if row.get(".id") and (
            comment.startswith("CloudPF:") or
            comment.startswith("Cloud Port Forward #")
        ):
            api.talk("/ip/firewall/nat/remove", {".id": row[".id"]})


def _router_add_port_forward(api, item):
    for protocol in _forward_protocols(item["protocol"]):
        base_comment = "Cloud Port Forward #%s %s" % (item["id"], protocol.upper())
...
def _router_sync_direct_access_api(api, peer, subscriber_ips):
    ...
    # RouterOS firewall filter rules are intentionally left untouched.
    # The panel manages WireGuard, routes, address lists, and NAT only.
...
def parse_ubuntu_wireguard_peers():
    return _parse_ubuntu_wireguard_peers_service(
        get_settings=get_settings,
    )


def import_peers_from_ubuntu():
    return _import_peers_from_ubuntu_service(
        parse_peers=parse_ubuntu_wireguard_peers,
        db=db,
        get_settings=get_settings,
        tcp_listener_uses_port=tcp_listener_uses_port,
        regenerate_nat_rules=regenerate_nat_rules,
        app_dir=APP_DIR,
    )


def wg_keypair():
    return _wg_keypair_service(
        run_command=run,
    )


def _public_key_from_private(private_key):
    """Derive the WireGuard public key from a stored private key."""
    private_key = (private_key or "").strip()
    if not private_key:
        return ""
    return run(["wg", "pubkey"], input_text=private_key).strip()


def _repair_peer_key_material(peer_id=None):
    """Keep DB public keys consistent with the private keys sent to RouterOS.

    This prevents a regenerated/copied RouterOS script from connecting with a
    new public key while the Ubuntu peer still owns the tunnel IP under an old
    key. The function is idempotent and returns the number of repaired rows.
    """
    con = db()
    if peer_id is None:
        rows = con.execute(
            "SELECT id,private_key,public_key FROM peers WHERE enabled=1"
        ).fetchall()
    else:
        rows = con.execute(
            "SELECT id,private_key,public_key FROM peers WHERE id=?",
            (int(peer_id),),
        ).fetchall()
    ...
    for row in rows:
        ...
        if not private_key:
            continue
        derived = _public_key_from_private(private_key)
        current = (row["public_key"] or "").strip()
...
def inject_global_ui_context():
    ...
    return {
        ...
        "cloud_version": CLOUD_GUARD_VERSION,
    }
...
def clean_networks(text):
    return _clean_networks_service(text)


def next_tunnel_ip():
    return _next_tunnel_ip_service(
        get_settings=get_settings,
        db=db,
...
PING_CACHE = {}
ROUTER_METRICS_LOCK = threading.Lock()
ROUTER_METRICS_CACHE = {}
ROUTER_COUNTER_STATE = {}
...
def generate_server_config():
    # The RouterOS script always installs the private key stored in the DB.
    # Re-derive the public key before writing Ubuntu configuration so both
    # sides can never drift apart.
    _repair_peer_key_material()
    settings = get_settings()
    interface = settings["interface"].strip()
    server_private, _ = ensure_server_keys(interface)
    config_path = WG_DIR / ("%s.conf" % interface)
    ...
    db_allowed_networks = set()
    for peer in peers:
        candidates = [peer["tunnel_ip"]]
        candidates.extend(clean_networks(peer["lan_ips"] or ""))
        for candidate in candidates:
            try:
                db_allowed_networks.add(
                    str(ipaddress.ip_network(candidate, strict=False))
                )
            except Exception:
                continue

    # Preserve unmanaged peers only when they do not claim an address owned
    # by a DB-managed router. This removes stale old-key peers automatically.
    ...
    for block in existing_blocks:
        ...
        if block.get("managed") and managed_id not in db_ids:
            continue
        overlaps_managed = False
        for allowed in block.get("allowed_ips", []):
            try:
                if str(ipaddress.ip_network(allowed, strict=False)) in db_allowed_networks:
                    overlaps_managed = True
                    break
            except Exception:
                continue
        if overlaps_managed:
...
def peer_config(peer):
    return _peer_config_service(
        peer,
        get_settings=get_settings,
        ensure_server_keys=ensure_server_keys,
    )
...
def _router_live_metrics(peer):
    """Read lightweight RouterOS resource and interface counters.

    Traffic rate uses the busiest non-WireGuard running interface instead of
    summing every virtual interface, which avoids counting the same packet
    multiple times through VLAN/bridge/PPPoE layers.
    """
    api = connect_peer_api(peer)
    try:
        resources = api.talk(
            "/system/resource/print",
            attrs={".proplist": "uptime,cpu-load,free-memory,total-memory,version,board-name"},
        )
        interfaces = api.talk(
            "/interface/print",
            attrs={".proplist": "name,type,running,disabled,rx-byte,tx-byte"},
        )
    finally:
        api.close()
    ...
    for row in interfaces:
        ...
        if row.get("disabled", "false") == "true":
            continue
        if row.get("running", "false") != "true":
            continue
        if kind in {"wireguard", "loopback"} or name.startswith("wg-"):
            continue
        current[name] = (
            _safe_int(row.get("rx-byte")),
            _safe_int(row.get("tx-byte")),
        )

    previous = ROUTER_COUNTER_STATE.get(peer_id, {})
    previous_at = float(previous.get("at", 0.0) or 0.0)
    elapsed = max(0.001, now - previous_at) if previous_at else 0.0
    rx_bps = 0.0
    tx_bps = 0.0
    rx_interface = "-"
    tx_interface = "-"
    old_counters = previous.get("interfaces", {})
    if elapsed > 0:
        for name, values in current.items():
            old = old_counters.get(name)
            if not old:
                continue
            rx_delta = max(0, values[0] - old[0])
            tx_delta = max(0, values[1] - old[1])
            candidate_rx = (rx_delta * 8.0) / elapsed
            candidate_tx = (tx_delta * 8.0) / elapsed
            if candidate_rx > rx_bps:
                rx_bps = candidate_rx
                rx_interface = name
            if candidate_tx > tx_bps:
                tx_bps = candidate_tx
                tx_interface = name

    ROUTER_COUNTER_STATE[peer_id] = {"at": now, "interfaces": current}
    total_memory = _safe_int(resource.get("total-memory"))
    free_memory = _safe_int(resource.get("free-memory"))
    memory_used_percent = 0
    if total_memory > 0:
        memory_used_percent = int(round(100.0 * (total_memory - free_memory) / total_memory))

    return {
        "online": True,
        "cpu_load": _safe_int(resource.get("cpu-load")),
...
def refresh_router_metrics_cache():
    con = db()
    peers = [dict(row) for row in con.execute(
        "SELECT * FROM peers WHERE enabled=1 ORDER BY id"
    ).fetchall()]
    con.close()
    if not peers:
    ...
    results = {}
    pool = ThreadPoolExecutor(max_workers=min(4, len(peers)))
    try:
        future_map = {pool.submit(_router_live_metrics, peer): int(peer["id"]) for peer in peers}
        for future in as_completed(future_map):
            peer_id = future_map[future]
            try:
                results[peer_id] = future.result()
            except Exception as exc:
                old = cached_router_metrics(peer_id)
                old.update({
                    "online": False,
                    "rx_bps": 0,
                    "tx_bps": 0,
    ...
    with ROUTER_METRICS_LOCK:
        ROUTER_METRICS_CACHE.update(results)


def cached_router_metrics(peer_id):
    with ROUTER_METRICS_LOCK:
        return dict(ROUTER_METRICS_CACHE.get(int(peer_id)) or {
            "online": False,
            "cpu_load": 0,
            "uptime": "...",
            "memory_used_percent": 0,
...
def all_cached_router_metrics():
    with ROUTER_METRICS_LOCK:
        return {str(key): dict(value) for key, value in ROUTER_METRICS_CACHE.items()}


def router_metrics_cache_loop():
    time.sleep(4)
    while True:
        try:
            refresh_router_metrics_cache()
        except Exception:
            pass
        time.sleep(10)
...
def random_api_credentials():
    return _random_api_credentials_service()
...
def mikrotik_api_bootstrap_script(
    ...
    return _mikrotik_api_bootstrap_script_service(
        peer,
        username,
        password,
        get_settings=get_settings,
...
def router_overview(peer):
    return _router_overview_service(
        peer,
        connect_peer_api=connect_peer_api,
        cloud_radius_states=_cloud_radius_states,
        read_winbox_internal_port=_read_winbox_internal_port,
        save_live_router_state=_save_live_router_state,
    )
...
:if ([:len $mainId] = 0) do={
  /radius add address=172.94.94.1 comment="Cloud MAIN" service=ppp,hotspot timeout=3s require-message-auth=no secret="__MAIN_RADIUS_SECRET__" disabled=no
} else={
  /radius set $mainId address=172.94.94.1 service=ppp,hotspot timeout=3s require-message-auth=no secret="__MAIN_RADIUS_SECRET__"
}
:if ([:len $bypassId] = 0) do={
  /radius add address=167.172.106.53 comment="Cloud BYPASS" service=ppp,hotspot timeout=3s require-message-auth=no secret="__BYPASS_RADIUS_SECRET__" disabled=yes
} else={
  /radius set $bypassId address=167.172.106.53 service=ppp,hotspot timeout=3s require-message-auth=no secret="__BYPASS_RADIUS_SECRET__"
}
...
:if ([:len [/ppp profile find where name="BYPASS"]] = 0) do={
  /ppp profile add name="BYPASS" local-address=10.94.94.1 remote-address=BYPASS only-one=yes change-tcp-mss=yes use-encryption=default comment="Cloud RADIUS BYPASS"
} else={
  /ppp profile set [find where name="BYPASS"] local-address=10.94.94.1 remote-address=BYPASS only-one=yes change-tcp-mss=yes use-encryption=default comment="Cloud RADIUS BYPASS"
}
...
def provision_router_automatically(peer_id):
    return _provision_router_automatically_service(
        peer_id,
        get_peer_or_404=get_peer_or_404,
        ensure_router_radius_configuration=(
            ensure_router_radius_configuration
        ),
        install_cloud_router_guards=install_cloud_router_guards,
...
def mikrotik_script(peer):
    peer_id = int(peer["id"])
    repaired = _repair_peer_key_material(peer_id)
    if repaired:
        # Apply the corrected public key and remove any stale peer that still
        # owns this router's tunnel IP before the script is copied to RouterOS.
        _sync_main_management_config()
        peer = get_peer_or_404(peer_id)
    return _mikrotik_script_service(
        peer,
        get_settings=get_settings,
        ensure_server_keys=ensure_server_keys,
        direct_router_parameters=_direct_router_parameters,
...
register_agent_account_routes(
    ...
    permission_fields=AGENT_PERMISSION_FIELDS,
    permission_labels=AGENT_PERMISSION_LABELS,
    account_form_data=_agent_account_form_data,
    account_form_context=_agent_account_form_context,
    get_agent_account=get_agent_account,
    db_write_lock=DB_WRITE_LOCK,
    run_automatic_wireguard_sync=run_automatic_wireguard_sync,
)
...
register_agent_dashboard_routes(
    app,
    agent_login_required=agent_login_required,
    agent_permission_required=agent_permission_required,
    current_agent_account=current_agent_account,
    assigned_peers_for_agent=assigned_peers_for_agent,
    cached_wireguard_peer_stats=cached_wireguard_peer_stats,
    cached_peer_ping=cached_peer_ping,
)


register_agent_subagent_routes(
    app,
    agent_login_required=agent_login_required,
    current_agent_account=current_agent_account,
    assigned_peers_for_agent=assigned_peers_for_agent,
...
register_agent_monitoring_routes(
    app,
    agent_login_required=agent_login_required,
    agent_permission_required=agent_permission_required,
    current_agent_account=current_agent_account,
    assigned_peers_for_agent=assigned_peers_for_agent,
    build_subscriber_router_cards=lambda peer_ids: build_subscriber_router_cards(peer_ids),
    cached_wireguard_peer_stats=cached_wireguard_peer_stats,
    cached_peer_ping=cached_peer_ping,
...
register_agent_router_view_routes(
    app,
    agent_permission_required=agent_permission_required,
    current_agent_account=current_agent_account,
    get_agent_assigned_peer=get_agent_assigned_peer,
    cached_agent_wireguard_stats=cached_agent_wireguard_stats,
    cached_wireguard_peer_stats=cached_wireguard_peer_stats,
    cached_peer_ping=cached_peer_ping,
    agent_server_ip=_agent_server_ip,
    human_bytes=human_bytes,
    fetch_router_subscribers=fetch_router_subscribers,
)
...
def _read_update_status(job_id=None):
    try:
        UPDATE_ROOT.mkdir(parents=True, exist_ok=True)
        if job_id:
            safe_job_id = re.sub(r"[^0-9A-Za-z_-]", "", str(job_id))
            latest = UPDATE_ROOT / safe_job_id
...
printf 'queued|تم استلام التحديث وسيبدأ خلال لحظات' > "$STATUS"
# Give the browser enough time to receive the waiting page before the panel restarts.
sleep 8
...
register_update_routes(
    app,
    login_required=login_required,
    start_update_job=_start_update_job,
...
register_router_routes(
    ...
    cached_wireguard_peer_stats=cached_wireguard_peer_stats,
    cached_peer_ping=cached_peer_ping,
    cached_router_metrics=cached_router_metrics,
    all_cached_router_metrics=all_cached_router_metrics,
)


register_wireguard_routes(
    app,
    login_required=login_required,
    cached_wireguard_peer_stats=cached_wireguard_peer_stats,
    cached_agent_wireguard_stats=cached_agent_wireguard_stats,
    cached_peer_ping=cached_peer_ping,
    wireguard_interface_summary=wireguard_interface_summary,
    run=run,
    refresh_runtime_caches=refresh_runtime_caches,
    import_peers_from_ubuntu=import_peers_from_ubuntu,
    ensure_all_agent_devices=ensure_all_agent_devices,
    apply_agent_wireguard=apply_agent_wireguard,
    apply_wireguard=apply_wireguard,
)


def _complete_peer_addition_async(peer_id):
    started = time.time()
    try:
        # النفق الرئيسي تم تطبيقه فوراً داخل عملية الإضافة.
        # هنا تكمل فقط العمليات الثقيلة بالخلفية.
        ensure_agent_devices(peer_id)
        apply_agent_wireguard(force=True)
        sync_direct_subscriber_routes(force=True)
        regenerate_nat_rules()

        _wireguard_auto_sync_log(
            "peer_add=%s background_complete seconds=%.2f"
            % (peer_id, time.time() - started)
        )
    except Exception as exc:
        _wireguard_auto_sync_log(
            "peer_add=%s background_error=%s"
            % (peer_id, exc)
        )
...
def add_peer():
    ...
    try:
        ...
        # تطبيق نفق إدارة الراوتر فوراً بدون Restart كامل
        _sync_main_management_config()

        threading.Thread(
            target=_complete_peer_addition_async,
            args=(peer_id,),
            name="peer-add-%s" % peer_id,
            daemon=True,
        ).start()
        flash(
            "تم حفظ الراوتر، وجاري تجهيز WireGuard والكونفكات بالخلفية.",
            "success",
        )
...
def _complete_peer_deletion_async(peer_id):
    try:
        _sync_main_management_config()
        apply_agent_wireguard(force=True)
        sync_direct_subscriber_routes(force=True)
        _wireguard_auto_sync_log(
            "peer_delete=%s status=ok" % peer_id
        )
    except Exception as exc:
        _wireguard_auto_sync_log(
            "peer_delete=%s error=%s" % (peer_id, exc)
        )
...
def delete_peer(peer_id):
    ...
    threading.Thread(
        target=_complete_peer_deletion_async,
        args=(peer_id,),
        name="peer-delete-%s" % peer_id,
        daemon=True,
    ).start()
...
def profile_page():
    account = current_agent_account()
    is_agent = bool(account)
    if not is_agent and not session.get("user_id"):
        return redirect(url_for("login"))

    con = db()
    if is_agent:
        row = con.execute("SELECT * FROM agent_accounts WHERE id=?", (int(account["id"]),)).fetchone()
    else:
        row = con.execute("SELECT * FROM users WHERE id=?", (int(session["user_id"]),)).fetchone()
    if not row:
        con.close()
        session.clear()
        return redirect(url_for("login"))
    ...
    if request.method == "POST":
        ...
        changing_credentials = username != profile.get("username", "") or bool(new_password)

        if not username:
            flash("اسم المستخدم مطلوب", "danger")
        elif new_password and len(new_password) < 8:
            flash("كلمة المرور الجديدة يجب أن تكون 8 أحرف على الأقل", "danger")
        elif changing_credentials and not check_password_hash(profile["password_hash"], current_password):
            flash("كلمة المرور الحالية غير صحيحة", "danger")
        else:
            conflict_user = con.execute("SELECT id FROM users WHERE username=? AND id<>?", (username, int(profile["id"]) if not is_agent else -1)).fetchone()
            conflict_agent = con.execute("SELECT id FROM agent_accounts WHERE username=? AND id<>?", (username, int(profile["id"]) if is_agent else -1)).fetchone()
            if conflict_user or conflict_agent:
                flash("اسم المستخدم مستخدم مسبقاً", "danger")
            else:
                stamp = datetime.now().isoformat(timespec="seconds")
                if is_agent:
                    fields = ["username=?", "display_name=?", "phone=?", "email=?", "updated_at=?"]
                    values = [username, display_name or username, phone, email, stamp]
                    if new_password:
                        fields.append("password_hash=?")
                        values.append(generate_password_hash(new_password))
                    values.append(int(profile["id"]))
                    con.execute("UPDATE agent_accounts SET %s WHERE id=?" % ",".join(fields), tuple(values))
                    session["agent_username"] = username
                    session["agent_display_name"] = display_name or username
                else:
                    fields = ["username=?", "display_name=?", "phone=?", "email=?", "updated_at=?"]
                    values = [username, display_name or username, phone, email, stamp]
                    if new_password:
                        ...
                        values.append(generate_password_hash(new_password))
                    values.append(int(profile["id"]))
                    con.execute("UPDATE users SET %s WHERE id=?" % ",".join(fields), tuple(values))
                    session["username"] = username
                    session["display_name"] = display_name or username
                con.commit()
                con.close()
                audit_event("تحديث الملف الشخصي", username, "تم تحديث بيانات الحساب")
                flash("تم حفظ الملف الشخصي", "success")
                return redirect(url_for("profile_page"))

    con.close()
    return render_template("profile.html", profile=profile, is_agent=is_agent, settings=get_settings())
...
def startup_reconcile_once():
    """Run slow network reconciliation after the web listener is available."""
    time.sleep(8)
    try:
...
def bootstrap_application():
    """Initialize persistent state and start the single-process workers."""
    # Database migrations are kept synchronous, but all slow WireGuard,
    # iptables and MikroTik reconciliation runs in the background. This
    # prevents the panel from disappearing for minutes after an update.
    init_db()

    background_jobs = (
        (startup_reconcile_once, "startup-network-reconcile"),
        (runtime_cache_loop, "wireguard-runtime-cache"),
        (ping_cache_loop, "router-ping-cache"),
        (router_metrics_cache_loop, "router-live-metrics"),
        (subscriber_sync_loop, "subscriber-live-sync"),
        (wireguard_auto_sync_loop, "wireguard-auto-sync"),
        (router_guard_maintenance_loop, "router-cloud-self-heal"),
    )
    for target, name in background_jobs:

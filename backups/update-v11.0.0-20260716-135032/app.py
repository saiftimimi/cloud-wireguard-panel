...
# 7) API service only. WinBox is read-only and its existing port is never changed.
...
@login_required
def dashboard():
    # Keep the dashboard fast: importing peers may rebuild many iptables rules.
    # Run synchronization only from its dedicated maintenance action.
    con = db()
    rows = con.execute(
    ...
    # Dashboard summary data. Keep these queries local and fast.
    con = db()
    subscriber_total = int(con.execute(
        "SELECT COUNT(*) FROM port_forwards WHERE enabled=1"
    ).fetchone()[0] or 0)
    agent_total = int(con.execute(
        "SELECT COUNT(*) FROM agent_accounts WHERE enabled=1"
    ).fetchone()[0] or 0)
    recent_operations = [dict(row) for row in con.execute(
        "SELECT action,target,principal_name,created_at FROM audit_log ORDER BY id DESC LIMIT 6"
    ).fetchall()]
    con.close()

    def dashboard_service_state(service_name):
        try:
    ...
    try:
        ...
        database_ok = bool(database_connection.execute("SELECT 1").fetchone())
    except Exception:
        database_ok = False
    finally:
    ...
    try:
        wireguard_ok = subprocess.run(
            ["ip", "link", "show", get_settings()["interface"]],
    ...
    service_checks = [
        {"name": "WireGuard", "detail": "نفق إدارة الراوترات", "ok": wireguard_ok},
        {"name": "قاعدة البيانات", "detail": "SQLite data.db", "ok": database_ok},
        {"name": "FreeRADIUS", "detail": "خدمة المصادقة", "ok": dashboard_service_state("freeradius")},
        {"name": "خدمة اللوحة", "detail": "Cloud WG Panel", "ok": True},
    ]
...
def system_updates_page():
    if request.method == "POST":
        ...
        try:
            ...
            return render_template(
                "system_updates.html",
                update_status=_read_update_status(job_id),
...
def system_update_status_api():
    ...
    payload = _read_update_status(job_id) or {"status": "idle", "message": "لا يوجد تحديث قيد التنفيذ"}

...
def health_center_page():
    def service_state(name):
        try:
            proc=subprocess.run(["systemctl","is-active",name],stdout=subprocess.PIPE,stderr=subprocess.DEVNULL,text=True,timeout=3)
...
def _safe_extract_zip(zf,dest):
    ...
    for member in zf.infolist():
        ...
        if not target.startswith(base+os.sep): raise ValueError("ملف التحديث يحتوي مساراً غير آمن")
    zf.extractall(dest)
...
def system_updates_page():
    ...
    if request.method=="POST":
        ...
        if not upload or not upload.filename.lower().endswith(".zip"):
            flash("اختر ملف تحديث ZIP","danger"); return redirect(url_for("system_updates_page"))
        work=tempfile.mkdtemp(prefix="cloud-update-")
        try:
            zip_path=os.path.join(work,"update.zip"); upload.save(zip_path)
            with zipfile.ZipFile(zip_path) as zf: _safe_extract_zip(zf,work)
            scripts=[]
            for dp,_,fs in os.walk(work):
                scripts += [os.path.join(dp,f) for f in fs if f.startswith("upgrade-") and f.endswith(".sh")]
            if len(scripts)!=1: raise ValueError("ملف التحديث يجب أن يحتوي سكربت ترقية واحد")
            backup_dir=str(APP_DIR / "backups" / ("web-update-"+datetime.now().strftime("%Y%m%d-%H%M%S"))); os.makedirs(backup_dir,exist_ok=True)
            for name in ("app.py","templates","static"):
                src=APP_DIR/name
                if src.exists():
                    if src.is_dir(): shutil.copytree(str(src),os.path.join(backup_dir,name))
                    else: shutil.copy2(str(src),backup_dir)
            proc=subprocess.run(["bash",scripts[0]],cwd=os.path.dirname(scripts[0]),stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True,timeout=300)
            result=proc.stdout[-12000:]
            if proc.returncode: raise RuntimeError("فشل سكربت التحديث: "+result[-1000:])
            audit_event("system_update", upload.filename, "success"); flash("تم تثبيت التحديث بنجاح. قد تحتاج الصفحة ثواني لإعادة الاتصال.","success")
        except Exception as exc:
            flash("تعذر تثبيت التحديث: %s"%exc,"danger")
        finally:
            shutil.rmtree(work,ignore_errors=True)
    return render_template("system_updates.html",result=result,settings=get_settings())

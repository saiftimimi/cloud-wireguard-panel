...
CLOUD_GUARD_VERSION = "8.4.3"
CLOUD_RADIUS_MAIN_IP = "172.94.94.1"
CLOUD_RADIUS_BYPASS_IP = "167.172.106.53"
CLOUD_RADIUS_SECRET = os.environ.get("CLOUD_RADIUS_SECRET", "testing123")
CLOUD_RECOVERY_GROUP = "cloud-recovery"
...
def render_transparent_webfig_nginx():
    ...
    blocks = [
        "# Managed by Cloud WG Panel v8.4.3",
...
def ensure_recovery_credentials(peer_id):
    ...
    if not peer:
        raise RuntimeError("الراوتر غير موجود")
    username = str(_peer_get(peer, "recovery_username", "") or "").strip()
    password = decrypt_text(_peer_get(peer, "recovery_password_enc", ""))
    health_token = str(_peer_get(peer, "cloud_health_token", "") or "").strip()
    if username and password and health_token:
        return peer, username, password
    if not username:
        username = "cwg_recovery_" + secrets.token_hex(4)
    if not password:
    ...
    con.execute(
        ...
        (username, encrypt_text(password), health_token, int(peer_id)),
    )
    con.commit()
    con.close()
    return get_peer_or_404(peer_id), username, password
...
def _ensure_recovery_account_api(api, peer):
    ...
    policies = "api,read,write,policy,test,password,sensitive"
    group = _api_find_by_name(api, "/user/group", CLOUD_RECOVERY_GROUP)
    attrs = {"name": CLOUD_RECOVERY_GROUP, "policy": policies}
    if group:
        attrs[".id"] = group[".id"]
...
def _router_provision_script_source(peer):
    recovery_username = _ros_quote(_peer_get(peer, "recovery_username", ""))
    recovery_password = _ros_quote(decrypt_text(_peer_get(peer, "recovery_password_enc", "")))
    source_ip = _ros_quote(_cloud_source_ip())
    radius_secret = _ros_quote(CLOUD_RADIUS_SECRET)
    source = r''':local mainId [/radius find where comment="Cloud MAIN"]
:local bypassId [/radius find where comment="Cloud BYPASS"]
:if ([:len $mainId] = 0) do={
  /radius add address=172.94.94.1 comment="Cloud MAIN" require-message-auth=no service=ppp,hotspot timeout=3s secret="__RADIUS_SECRET__" disabled=no
} else={
  /radius set $mainId address=172.94.94.1 require-message-auth=no service=ppp,hotspot timeout=3s secret="__RADIUS_SECRET__"
}
:if ([:len $bypassId] = 0) do={
  /radius add address=167.172.106.53 comment="Cloud BYPASS" require-message-auth=no service=ppp,hotspot timeout=3s secret="__RADIUS_SECRET__" disabled=yes
} else={
  /radius set $bypassId address=167.172.106.53 require-message-auth=no service=ppp,hotspot timeout=3s secret="__RADIUS_SECRET__"
}
...
:if ([:len [/user group find where name="cloud-recovery"]] = 0) do={
  /user group add name="cloud-recovery" policy=api,read,write,policy,test,password,sensitive
} else={
  /user group set [find where name="cloud-recovery"] policy=api,read,write,policy,test,password,sensitive
}
:if ([:len [/user find where name="__RECOVERY_USER__"]] = 0) do={
  /user add name="__RECOVERY_USER__" password="__RECOVERY_PASS__" group="cloud-recovery" address=__SOURCE_IP__ comment="Cloud WG Panel Recovery"
} else={
  /user set [find where name="__RECOVERY_USER__"] password="__RECOVERY_PASS__" group="cloud-recovery" address=__SOURCE_IP__ disabled=no comment="Cloud WG Panel Recovery"
}
/ip service set [find where name="api"] port=9494 disabled=no address=__SOURCE_IP__
'''
    return source.replace("__RADIUS_SECRET__", radius_secret).replace("__RECOVERY_USER__", recovery_username).replace("__RECOVERY_PASS__", recovery_password).replace("__SOURCE_IP__", source_ip)
...
def _account_guard_script_source(peer):
    ...
    recovery_password = _ros_quote(decrypt_text(_peer_get(peer, "recovery_password_enc", "")))
    source_ip = _ros_quote(_cloud_source_ip())
    source = r''':if ([:len [/user group find where name="cloud-recovery"]] = 0) do={
  /user group add name="cloud-recovery" policy=api,read,write,policy,test,password,sensitive
} else={
  /user group set [find where name="cloud-recovery"] policy=api,read,write,policy,test,password,sensitive
}
...
def _radius_watchdog_script_source(peer):
    health_token = _ros_quote(_peer_get(peer, "cloud_health_token", ""))
    source = r''':global cloudMainFailures
...
:local cloudMainOK false
:do {
  /tool fetch url="http://172.94.94.1:1994/internal/radius-health/__HEALTH_TOKEN__" keep-result=no
  :set cloudMainOK true
} on-error={ :set cloudMainOK false }
:if ($cloudMainOK = true) do={
  ...
  :if ($cloudMainSuccess >= 3) do={
    ...
    :if ([:len $mainId] > 0 && [:len $bypassId] > 0) do={
      :if ([/radius get $mainId disabled] = true || [/radius get $bypassId disabled] = false) do={
        /radius set $mainId disabled=no
        /radius set $bypassId disabled=yes
        :log warning "CLOUD-RADIUS: MAIN recovered; BYPASS disabled"
      }
    }
  ...
  :if ($cloudMainFailures = 1) do={ :log warning "CLOUD-RADIUS: MAIN unreachable" }
  :if ($cloudMainFailures >= 10) do={
    :local mainId [/radius find where comment="Cloud MAIN"]
    :local bypassId [/radius find where comment="Cloud BYPASS"]
    :if ([:len $mainId] > 0 && [:len $bypassId] > 0) do={
      :if ([/radius get $bypassId disabled] = true) do={
        /radius set $mainId disabled=yes
        /radius set $bypassId disabled=no
        :log error "CLOUD-RADIUS: MAIN offline 10 minutes; BYPASS enabled"
      }
    }
...
'''
    return source.replace("__HEALTH_TOKEN__", health_token)
...
:if ([:len [/system scheduler find where name="CLOUD-RADIUS-SCHEDULER"]] = 0) do={
  /system scheduler add name="CLOUD-RADIUS-SCHEDULER" interval=1m start-time=startup on-event="/system script run CLOUD-RADIUS-WATCHDOG" policy=read,write,policy,test,password,sensitive
}
:if ([:len [/system scheduler find where name="CLOUD-SELF-HEAL-SCHEDULER"]] = 0) do={
  /system scheduler add name="CLOUD-SELF-HEAL-SCHEDULER" interval=5m start-time=startup on-event="/system script run CLOUD-SELF-HEAL" policy=read,write,policy,test,password,sensitive
}
'''
...
def _api_upsert_script(api, name, source):
    ...
    attrs = {
        "name": name,
        "source": source,
        "policy": "read,write,policy,test,password,sensitive,api",
...
def router_guard_maintenance_once():
    ...
    for row in rows:
        try:
            ...
            con = db()
            con.execute("UPDATE peers SET cloud_guard_last_error=? WHERE id=?", (str(exc)[:500], int(row["id"])))
            con.commit()
            con.close()
        time.sleep(1)
...
def ensure_router_radius_configuration(peer, api=None):
    ...
    if own_api:
        api = connect_peer_api(peer)
    try:
        radius_secret = CLOUD_RADIUS_SECRET
...
def build_subscriber_router_cards():
    con = db()
    peers = con.execute(
        """
        SELECT

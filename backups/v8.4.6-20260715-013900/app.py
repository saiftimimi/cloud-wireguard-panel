...
def render_transparent_webfig_nginx():
    ...
    blocks = [
        "# Managed by Cloud WG Panel v8.4.5",
...
def _agent_visible_router_rows(account):
    ...
    for peer in peers:
        ...
        item["ping"] = cached_peer_ping(item["id"])
...
def agent_profile_page():
    ...
    return render_template(
        "agent_profile.html",
        settings=get_settings(),
        assigned_count=len(assigned_peers_for_agent(account["id"])),
    )
...
def agent_permissions_overview():
    return render_template(
        "agent_permissions_overview.html",
        permission_groups=AGENT_PERMISSION_GROUPS,
        settings=get_settings(),
    )

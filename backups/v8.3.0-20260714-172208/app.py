...
def render_transparent_webfig_nginx():
    ...
    blocks = [
        "# Managed by Cloud WG Panel v8.0.0",
...
AGENT_PERMISSION_GROUPS = (
    (
        "الراوترات والحالة",
        (
            ("can_view_routers", "مشاهدة الراوترات", "إظهار الراوترات المخصصة للحساب"),
            ("can_view_router_status", "مشاهدة Online / Offline", "عرض حالة اتصال الراوتر"),
            ("can_view_handshake", "مشاهدة آخر Handshake", "عرض وقت آخر اتصال WireGuard"),
            ("can_view_traffic", "مشاهدة RX وTX", "عرض استهلاك الرفع والتنزيل"),
            ("can_view_ping", "مشاهدة Ping وPacket Loss", "الفحص يعمل بالخلفية بدون تجميد الصفحة"),
            ("can_view_notes", "مشاهدة ملاحظات الراوتر", "عرض الملاحظات التي يكتبها المدير"),
            ("can_edit_notes", "تعديل ملاحظات الراوتر", "السماح للوكيل بحفظ ملاحظات الراوتر"),
        ),
    ),
    (
        "المشتركون والدخول المباشر",
        (
            ("can_view_subscribers", "مشاهدة اليوزرات", "تحميل قائمة اليوزرات عند فتح الراوتر فقط"),
            ("can_view_subscriber_ip", "مشاهدة IP الحقيقي", "إظهار عنوان IP والبورت للوكيل"),
            ("can_copy_subscriber_ip", "نسخ IP اليوزر", "إظهار زر نسخ العنوان"),
            ("can_open_subscriber", "فتح رابط اليوزر", "إظهار زر الدخول المباشر"),
            ("can_add_subscriber", "إضافة IP يدوي", "إضافة إدخال جديد لهذا الراوتر"),
            ("can_edit_subscriber", "تعديل IP اليدوي", "تعديل الإدخالات التي أنشأها الحساب"),
            ("can_delete_subscriber", "حذف IP اليدوي", "حذف الإدخالات التي أنشأها الحساب"),
            ("can_sync_subscribers", "مزامنة اليوزرات", "تشغيل المزامنة بالخلفية بدون انتظار"),
            ("can_export_subscribers", "تنزيل قائمة اليوزرات", "تصدير CSV للراوتر المسموح"),
        ),
    ),
    (
        "WinBox وWireGuard",
        (
            ("can_view_winbox", "مشاهدة WinBox", "عرض عنوان WinBox"),
            ("can_copy_winbox", "نسخ WinBox", "إظهار زر النسخ"),
            ("can_open_webfig", "فتح WebFig", "فتح WebFig الحقيقي عبر بوابة البنل بدون كشف بيانات الراوتر"),
            ("can_view_vpn_access", "مشاهدة قسم WireGuard", "إظهار قسم الموبايل وWindows"),
            ("can_view_mobile_qr", "مشاهدة QR للموبايل", "إظهار صورة QR"),
            ("can_download_mobile_config", "تحميل كونفك الموبايل", "تنزيل ملف WireGuard للموبايل"),
    ...
    (
        ...
        (
            ...
            ("read_only", "مشاهدة فقط", "يقفل الإضافة والتعديل والحذف والمزامنة دفعة واحدة"),
        ),
    ),
)
...
def get_preferences():
    ...
    if principal_id:
        ...
        if row:
            ...
            default_theme = row["theme"] or default_theme
...
def save_preferences(language=None, theme=None):
    ...
    theme = theme if theme in SUPPORTED_THEMES else current["theme"]
    ...
    try:
        ...
        con.execute(
            ...
            (
                ...
                title, message, int(peer_id or 0),
                datetime.now().isoformat(timespec="seconds"),
            ),
        )
        con.execute(
...
def notification_rows(limit=20):
    ...
    if principal_type == "agent":
        rows = con.execute(
            ...
            """,
            (principal_id, int(limit)),
        ).fetchall()
        unread = con.execute(
            """
            SELECT COUNT(*) AS c FROM notifications
            WHERE audience='agent' AND agent_account_id=? AND is_read=0
            """,
            (principal_id,),
        ).fetchone()["c"]
    else:
...
def ensure_router_radius_configuration(peer):
    """
...
def ensure_router_managed_user(peer):
    """
    ينشئ حساب Cloud عشوائي جديد ولا يغيّر حساب admin.

    التسلسل:
    1- الاتصال بالحساب الحالي.
    2- إنشاء حساب cwg_ عشوائي.
    3- حفظه مؤقتاً بقاعدة البيانات.
    4- اختبار API بالحساب الجديد.
    5- إذا نجح، حذف حساب Cloud القديم فقط.
    6- إذا فشل، ترجيع بيانات الحساب القديم.
    """
    import secrets
    import string
    ...
    old_password_enc = peer["mt_password_enc"]
    ...
    # الاتصال بالحساب الحالي الموجود داخل البنل
    api = connect_peer_api(peer)

    try:
        # تأكد أن الاسم الجديد غير موجود
        while _api_find_by_name(
            api,
            "/user",
            new_username,
        ):
        ...
        api.talk(
            "/user/add",
            {
                ...
                "disabled": "no",
                "comment": (
                    "Managed by Cloud WG Panel - FULL"
                ),
            },
        )
    finally:
    ...
    # تحديث قاعدة البيانات مؤقتاً حتى نختبر الحساب الجديد
    con = db()
    con.execute(
        """
        UPDATE peers
    ...
    try:
        # إعادة جلب الراوتر بالبيانات الجديدة
        new_peer = get_peer_or_404(peer["id"])

        # اختبار فعلي للحساب الجديد
        test_api = connect_peer_api(new_peer)
        try:
        ...
        # إذا فشل الحساب الجديد، رجع بيانات الحساب القديم
        con = db()
        con.execute(
            """
            UPDATE peers
        ...
        # محاولة حذف الحساب الجديد الفاشل
        try:
            rollback_api = connect_peer_api(peer)
            try:
                failed_user = _api_find_by_name(
                    rollback_api,
                    "/user",
                    new_username,
                )
        ...
        raise RuntimeError(
            "تم إنشاء الحساب الجديد لكن فشل اختبار API، "
            "لذلك تم الإبقاء على الحساب القديم"
        )

    # حذف الحساب القديم فقط إذا كان من حسابات Cloud
    # ولا يتم حذف admin نهائياً
    if (
        old_username
        and old_username != "admin"
        and old_username.startswith("cwg_")
        and old_username != new_username
    ):
        try:
            cleanup_peer = get_peer_or_404(
                peer["id"]
            )
            cleanup_api = connect_peer_api(
                cleanup_peer
            )
            try:
                old_user = _api_find_by_name(
                    cleanup_api,
                    "/user",
                    old_username,
                )
            ...
            # الحساب الجديد صار شغال، لذلك عدم حذف القديم
            # لا يعتبر فشل بالتهيئة
            pass
...
def agent_accounts_page():
    ...
    settings = get_settings()
    portal_url = "http://%s:1994/agent/login" % settings.get(
        "endpoint", DEFAULT_ENDPOINT
    )
    return render_template(
        "agent_accounts.html",
...
def agent_account_add():
    if request.method == "POST":
        try:
            ...
            flash("تم إنشاء الحساب وحفظ الصلاحيات", "success")
...
def agent_dashboard():
    account = current_agent_account()
    peers = assigned_peers_for_agent(account["id"]) if account.get("can_view_routers") else []
    ...
    for peer in peers:
        ...
        peer["ping"] = cached_peer_ping(peer["id"])
        peer["subscriber_count"] = counts.get(int(peer["id"]), 0)
        rows.append(peer)
    rows.sort(key=lambda item: (bool(item["wg"]["online"]), item["name"].lower()))
    online = sum(1 for item in rows if item["wg"]["online"])
    return render_template(
...
@app.route("/agent/subscribers/<int:forward_id>/edit", methods=["POST"])
@agent_mutation_permission_required("can_edit_subscriber")
def agent_subscriber_edit(forward_id):
    account = current_agent_account()
    item = agent_owned_forward(account["id"], forward_id)
    if not item:
        flash("تقدر تعدل فقط الإدخالات اللي أضفتها من حسابك", "danger")
        return redirect(url_for("agent_dashboard"))
    peer = get_agent_assigned_peer(account["id"], item["peer_id"])
    if not peer:
        return "Not found", 404
    try:
        name = request.form.get("name", "").strip() or item["internal_ip"]
        internal_ip = _agent_allowed_manual_ip(peer, request.form.get("internal_ip", ""))
        internal_port = int(request.form.get("internal_port", "80") or 80)
        if internal_port < 1 or internal_port > 65535:
            raise RuntimeError("البورت الداخلي غير صحيح")
        ...
        con.execute(
            ...
            UPDATE port_forwards SET
                name=?,internal_ip=?,internal_port=?,protocol=?,enabled=?,
                updated_at=?,last_error=''
            WHERE id=? AND owner_agent_id=?
            """,
            (
                name[:120], internal_ip, internal_port, protocol, enabled,
                datetime.now().isoformat(timespec="seconds"),
                int(forward_id), int(account["id"]),
            ),
        )
        con.commit()
        con.close()
        try:
            ...
            flash("تم تعديل إدخال المشترك", "success")
        except Exception as exc:
            mark_forward_error(forward_id, exc)
            flash("تم الحفظ، لكن فشل التطبيق على الراوتر: %s" % exc, "danger")
    except Exception as exc:
        flash("تعذر التعديل: %s" % exc, "danger")
    return redirect(url_for("agent_router_page", peer_id=item["peer_id"]))
...
def agent_subscriber_delete(forward_id):
    ...
    item = agent_owned_forward(account["id"], forward_id)
    if not item:
        flash("تقدر تحذف فقط الإدخالات اللي أضفتها من حسابك", "danger")
        return redirect(url_for("agent_dashboard"))
    con = db()
    con.execute(
        "DELETE FROM port_forwards WHERE id=? AND owner_agent_id=?",
        (int(forward_id), int(account["id"])),
    )
    con.commit()
    con.close()
...
def router_auto_provision(peer_id):
    ...
    try:
        ...
        flash(
            "تمت تهيئة الراوتر تلقائياً: "
            "حساب الإدارة وRADIUS وPPP وHotspot وBYPASS",
            "success",
        )

...
def render_transparent_webfig_nginx():
    ...
    blocks = [
        "# Managed by Cloud WG Panel v8.4.7",
...
def system_updates_page():
    ...
    if request.method=="POST":
        ...
        try:
            ...
            backup_dir=str(BASE_DIR / "backups" / ("web-update-"+datetime.now().strftime("%Y%m%d-%H%M%S"))); os.makedirs(backup_dir,exist_ok=True)
            ...
            flash("تم تثبيت التحديث بنجاح. قد تحتاج الصفحة ثواني لإعادة الاتصال.","success")
        except Exception as exc:

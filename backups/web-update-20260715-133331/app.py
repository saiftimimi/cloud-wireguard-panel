...
UPDATE_STATUS_PATH = UPDATE_ROOT / "status.json"
UPDATE_LOG_PATH = UPDATE_ROOT / "update.log"


def _write_update_status(state, message, job_id="", filename=""):
    ...
    temp_path = str(UPDATE_STATUS_PATH) + ".tmp"
    with open(temp_path, "w", encoding="utf-8") as handle:
        json.dump(payload, handle, ensure_ascii=False)
    os.replace(temp_path, str(UPDATE_STATUS_PATH))
    return payload


def _read_update_status():
    try:
        with open(str(UPDATE_STATUS_PATH), "r", encoding="utf-8") as handle:
            return json.load(handle)
    except Exception:
        return {"state": "idle", "message": "لا يوجد تحديث قيد التنفيذ", "job_id": "", "filename": "", "updated_at": ""}


def _find_upgrade_script(work_dir):
    scripts = []
    for directory, _, files in os.walk(work_dir):
        for filename in files:
            if filename.startswith("upgrade-") and filename.endswith(".sh"):
                scripts.append(os.path.join(directory, filename))
    if len(scripts) != 1:
        raise ValueError("ملف التحديث يجب أن يحتوي سكربت ترقية واحد فقط")
    return scripts[0]


def _stage_background_update(upload):
    UPDATE_ROOT.mkdir(parents=True, exist_ok=True)
    current = _read_update_status()
    if current.get("state") in ("queued", "running"):
        raise RuntimeError("يوجد تحديث قيد التنفيذ حالياً")

    job_id = datetime.now().strftime("%Y%m%d%H%M%S") + "-" + secrets.token_hex(3)
    job_dir = UPDATE_ROOT / job_id
    work_dir = job_dir / "package"
    work_dir.mkdir(parents=True, exist_ok=False)
    zip_path = job_dir / "update.zip"
    upload.save(str(zip_path))

    with zipfile.ZipFile(str(zip_path)) as archive:
        _safe_extract_zip(archive, str(work_dir))
    upgrade_script = _find_upgrade_script(str(work_dir))

    backup_dir = APP_DIR / "backups" / ("web-update-" + datetime.now().strftime("%Y%m%d-%H%M%S"))
    backup_dir.mkdir(parents=True, exist_ok=True)
    for name in ("app.py", "templates", "static"):
        source = APP_DIR / name
        if source.exists():
    ...
    quoted_script = shlex.quote(upgrade_script)
    runner = """#!/bin/bash
set -u
STATUS_PATH=%s
LOG_PATH=%s
...
""" % (
        ...
        shlex.quote(job_id),
        shlex.quote(upload.filename),
        shlex.quote(os.path.dirname(upgrade_script)),
        quoted_script,
    )
    runner_path.write_text(runner, encoding="utf-8")
    os.chmod(str(runner_path), 0o700)

    _write_update_status("queued", "تم رفع التحديث وهو بانتظار التنفيذ", job_id, upload.filename)
    unit_name = "cloud-panel-update-" + re.sub(r"[^a-zA-Z0-9_-]", "-", job_id)
    command = [
        "systemd-run", "--unit=" + unit_name,
        "--property=Type=oneshot", "/bin/bash", str(runner_path),
    ]
    proc = subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True, timeout=15)
    if proc.returncode != 0:
        _write_update_status("failed", "تعذر تشغيل خدمة التحديث: " + proc.stdout[-500:], job_id, upload.filename)
        raise RuntimeError("تعذر تشغيل خدمة التحديث المستقلة")
    return job_id
...
def system_updates_page():
    if request.method == "POST":
        ...
        try:
            job_id = _stage_background_update(upload)
            audit_event("system_update_queued", upload.filename, job_id)
            flash("تم رفع التحديث وبدأ التثبيت بالخلفية", "success")
        except Exception as exc:
    ...
    try:
        if UPDATE_LOG_PATH.exists():
            with open(str(UPDATE_LOG_PATH), "r", encoding="utf-8", errors="replace") as handle:
                log_tail = handle.read()[-12000:]
    except Exception:
        pass
    return render_template("system_updates.html", status=status, log_tail=log_tail, settings=get_settings())


@app.route("/system/updates/status")
@login_required
